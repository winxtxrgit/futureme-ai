...
| Career Decision Self-Efficacy | **Deferred** | Strong construct, but CDSE and CDSE-SF items are licensed commercially and cannot be reproduced. |
| Task self-efficacy per RIASEC dimension | **Adopted, 6 researcher-written items** | The deferral above rested on two things: the licence, and that no output branched on efficacy. The licence still bars CDSE, so these six are written from Bandura's (1977) task-confidence formulation rather than reproduced from an instrument. The second reason has lapsed — the programme recommender now carries efficacy as a second axis (`lib/recommend/`, 30% of CoreFit) and uses it to separate *interested but unpractised* from *capable but uninterested*, so the answers change what a learner is shown. They are unvalidated, live in their own `efficacy` array so they can never be summed into a RIASEC score, and are listed as a limitation in §12. |
