...
→ The AI asks from a future-scene prompt and folds evidence back into the current chapter.
→ editable evidence summary
→ choose a scenario mission
→ mission debrief
→ Side-by-side future scenes with assumptions, evidence, and trade-offs rather than match scores.
→ compare three route strategies
→ Horizontal DAG with zoomable years and a compact vertical version on mobile.
→ save
