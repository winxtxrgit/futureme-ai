...
→ A compact AI sidecar asks for missing evidence while the graph updates live.
→ editable evidence summary
→ choose a scenario mission
→ mission debrief
→ Three highlighted paths through the same network, each with different bridge skills.
→ compare three route strategies
→ Graph route with prerequisite edges and alternate bridge nodes.
→ save
