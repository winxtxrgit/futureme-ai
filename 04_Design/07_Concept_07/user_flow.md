...
→ Questions appear between story beats as a director’s debrief rather than a chat window.
→ editable evidence summary
→ choose a scenario mission
→ mission debrief
→ A narrative dossier linking choices to behavior evidence and three possible next chapters.
→ compare three route strategies
→ A chapter arc where each milestone is a scene, project, or real conversation.
→ save
...
Branching narrative choices followed by Socratic debrief and a replay-from-another-perspective option.
