...
<p align="center">
  <b>A student answers a few questions, tries one real task, and gets up to three study routes —<br>each showing the evidence behind it, and what it still doesn't know.</b>
</p>
...
<p align="center">
  <a href="#what-is-futureme-ai">What it is</a> ·
  <a href="#the-problem">The problem</a> ·
  <a href="#try-it-now">Try it</a> ·
  <a href="#what-you-can-try-today">What works today</a> ·
...
> **Runnable prototype.** The whole guest journey works end to end — no account, no API key. The
> output is for *exploration*, and has not been clinically, educationally or statistically
> validated. [What's real vs. what's designed →](#what-you-can-try-today)

---

## What is FutureMe AI?

FutureMe AI helps a Thai student explore where a study choice might lead — **before** they commit to
it. Instead of a test that prints a single answer, it gathers two kinds of evidence, then lays out a
few honest options the student can argue with.

It is decision *support*, not a prediction. It never names a winner, and it says so when it doesn't
know enough to suggest anything at all.

### The idea in five steps

<table>
<tr>
<td align="center" width="20%"><b>1 · Ask</b><br><sub>A short questionnaire forms a first guess about interests</sub></td>
<td align="center" width="20%"><b>2 · Try</b><br><sub>One small mission produces evidence from what you <i>do</i></sub></td>
<td align="center" width="20%"><b>3 · Compare</b><br><sub>Up to three routes, side by side, never ranked</sub></td>
<td align="center" width="20%"><b>4 · Explore</b><br><sub>Each route shows its evidence, limits and unknowns</sub></td>
<td align="center" width="20%"><b>5 · Act</b><br><sub>A 30-day plan of small, reversible experiments</sub></td>
</tr>
</table>

The point of step 2 is disagreement: saying you like design and *solving* a design problem are not
the same claim. When they conflict, the student is shown the conflict — that is usually the most
useful moment in the session.
...
Parents and counsellors are *designed* to read consented summaries only, never the free text. That
part is **designed, not built** — this prototype has no accounts.
...
Thai students pick a study track years before anyone helps them see where it leads. The bill arrives
later — and it is not unemployment. Graduate joblessness is low (**2.0%**), but **56% of Thai people
educated above upper-secondary work outside their field, and 27% below their qualification level**
(TDRI, 2025). The problem is *direction*, and the reasoning behind a direction is usually invisible
until it is too late to argue with.

FutureMe AI exists to make that reasoning visible early. → [The full evidence base](#research)
...
Click **Start as guest**, then follow: **interview → mission → routes → compare → 30-day plan.**

<p align="center">
  <a href="#what-you-can-try-today"><img src="assets/screenshots/app/routes-desktop.png" alt="Three route results in the running prototype — equal weight, no winner" width="90%"></a>
</p>

<p align="center">
<sub>The routes screen from the running app — equal visual weight, no winner, evidence and unknowns on<br>every card. Here the engine has detected that the interview and the mission disagree.</sub>
</p>

### Three things worth trying

Most demos only show the happy path. These show the engine's judgement.
...
Everything here is backed by code in this repository and by a test you can run. The distinction
between *built* and *designed* is kept deliberately sharp.

### What works today

- ✅ Start as a guest — no account — and recover the session after a refresh
- ✅ Complete the interview, with progress, validation and editable answers
- ✅ Get a mission chosen from your answers by a stated rule — and overrule it
- ✅ Receive **zero to three** explainable routes: never padded, never ranked
- ✅ See each route's evidence, limitations, unknowns and data provenance
- ✅ Compare routes on consistent criteria, then get a 30-day plan with check-ins
- ✅ Watch the engine refuse to answer rather than guess
- ✅ Run with **no LLM key**, delete all your data, and run 136 + 18 tests

<details>
<summary><b>What it does <em>not</em> do yet — read this, because a working demo hides it</b></summary>
...
<details>
<summary><b>Where each capability lives in the code</b></summary>

<br>

| Capability | Status | Where |
|---|:--|---|
| Guest session + refresh recovery | 🟢 | `lib/session/` |
| Interview | 🟡 static, English, unvalidated | `app/interview/` |
| Rule-based mission selection, overridable | 🟢 | `lib/mission/` |
| Mission draft autosave | 🟢 | `app/mission/` |
| Decision engine — 0–3 routes, ties marked | 🟢 | `lib/decision-engine/` |
| Refusal gates | 🟢 | `lib/decision-engine/` |
| Data provenance + freshness | 🟢 | `data/routes.json`, `app/routes/` |
| Optional AI explanation (wording only) | 🟢 | `app/api/explain/` |
| Safeguarding pause | 🟡 keyword only | `lib/safety/` |
| 30-day plan | 🟡 linear template | `lib/plan/`, `app/plan/` |
| Adaptive Thai interview · retrieval · DAG roadmap · accounts | 📐 planned | — |
...
Two independent streams of evidence feed one deterministic engine. **No model chooses anything.**
...
The rule walks your interest dimensions strongest-first and takes the first mission that lists that
dimension in its `bestFor`. Catalogue order breaks ties, so the same interview always gives the same
mission. It states its reason on screen, and it **declines to choose** — falling back to a default
and saying so — when the interview is too short or too flat to justify a choice. You can always pick
a different mission.
...
<td width="50%"><a href="assets/screenshots/app/plan-desktop.png"><img src="assets/screenshots/app/plan-desktop.png" alt="30-day plan in the running prototype"></a><br><b>30-day plan</b><br><sub>Weekly objectives, check-ins that persist, and tasks injected for the gaps the engine found.</sub></td>
</tr>
</table>

<details>
<summary><b>Mobile, safety, privacy and the no-route state</b></summary>
...
<td width="33%"><a href="assets/screenshots/app/insufficient-desktop.png"><img src="assets/screenshots/app/insufficient-desktop.png" alt="No-route state"></a><br><sub><b>No-route state</b> · refuses to guess</sub></td>
</tr>
<tr>
<td><a href="assets/screenshots/app/safety-desktop.png"><img src="assets/screenshots/app/safety-desktop.png" alt="Safeguarding pause screen"></a><br><sub><b>Safeguarding pause</b> · recommendations stop</sub></td>
...
<td><a href="assets/screenshots/app/plan-mobile.png"><img src="assets/screenshots/app/plan-mobile.png" alt="Plan on mobile"></a><br><sub><b>30-day plan</b> · mobile</sub></td>
</tr>
</table>
...
Everything happens in the browser. There is no server-side storage, which is what makes the
guest-mode privacy claim literally true rather than a policy promise.
...
**None of this is running.** It is recorded because the reasoning is part of the submission. Even
with retrieval and generation in place, the rule engine still decides — the model's job grows, but
it never gains the decision.
...
AIS Cloud's published specifications list Thai data centres, ISO 27001 / 27017 / 27018, CSA-STAR and
dSURE Cloud 3-star. Load is bursty and predictable — a counsellor runs a session with a whole class
at once — which suits container auto-scaling.

> **Status.** Designed from published specifications. **Nothing is deployed there**, and no capacity
> or latency figures have been measured.
...
guidance layer is a static RIASEC test, and used that as the product's justification. The July 2026
source audit could not verify it, so the claim is withdrawn — as is "DEEP provides national SSO".
The honest position is **aligned with national policy direction, not connected to it**.
...
The users are minors, so both are architectural constraints rather than policy pages.
...
| Corrupt state cannot poison the engine | Stored data is rebuilt field by field against the seed data; unrecognised values are dropped |
...
> **The distinction that matters.** AIS Cloud provides in-country residency and certification — where
> data lives and how the facility is run. It does **not** deliver PDPA compliance by itself. Consent,
> access control, minimisation, retention and processor governance are application-layer
> responsibilities. Conflating the two is the most common way a project like this gets compliance
> wrong. The safeguarding rule is a keyword match, not a risk assessment — a stopgap, listed as one.
...
The research base was re-audited on **24 July 2026**: every external URL opened and followed, every
figure compared against the page it came from.
...
and a "15–20% wage penalty". Three claims this repository *used as arguments* did not survive: that
NDLP's guidance layer is a static RIASEC test, that DEEP provides national SSO, and a fixed count of
"12 ปวช. subject areas". A test now fails if the withdrawn figures reappear on the product surface.
...
| **3 · Validation** | Next | Independent review of the instrument |
| **4 · School pilot** | Post-hackathon | Evidence students made decisions they can still defend |
| **5 · Ecosystem** | Long-term | Signed partnerships |

Phase 3 is the honest work: validating the instrument, replacing the illustrative catalogue with
licensed per-programme data, building an evaluation set, running a bias audit, and making the whole
thing work in Thai.
...
Everything below runs on every pull request. The demo needs **no environment variables and no API
key**.
...
| `npm run test:e2e` | Playwright against the production build | ✅ 18 passed |
...
instead — same tests: `PW_CHANNEL=chrome npm run test:e2e`
...
Nine documents, deepest detail last. Start with whichever question you arrived with.
...
**This is a student hackathon project** for JUMP Thailand Hackathon 2026 (AIS Academy × NIA).
Corrections to the research base are especially welcome — ten unsupported statistics have already
been removed. [Open an issue](https://github.com/winxtxrgit/futureme-ai/issues) or see
